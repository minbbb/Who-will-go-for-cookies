...
variationsRU = ("{} бежит за печенюхами", "{} ковыляет за печеньем", "{} стремительно мчится за кукисами", "{} в слоумо уносится за печенбками", "Собирая своё снаряжение {} уходит в пустоши сражатся с крысами мутантами за крышки, на которые он купит печенье")
variationsEN = ("{} running for cookies", "{} goes in slow motion for cookies")

def start(update, context):
	if update.message.from_user.language_code == "ru":
		context.bot.send_message(chat_id=update.message.chat_id, text="Отправь мне имена кандидатов на поход за печеньками через пробел.\nИ я скажу кто пойдёт за печеньем))0)\nЛибо можещь добавить меня в группу и делать тоже самое добавляя в начале /cookie (пример - /cookie имя1 имя2). Или же использовать меня в inline режиме, для этого в любом чате набери @Who_will_go_for_cookies_bot и имена через пробел.")
	else:
